var Backbone = require("modules-common/backbone/backbone.js"),
	...
	groupView = require("./group-select/group-select.js");


var PeopleSelectView = Backbone.View.extend({
	template: __inline("people-group-select.tmpl"),
	initialize: function() {
		this.render();
		this.initEvent();
	},
	...
	loadData: function(peopleList) {
		var that = this;
		this.$list.html("");
		this.$group = $("<div class='select-group JS-group'>选择讨论组<span></span></div>");
		this.$list.append(this.$group);
	...
	initEvent: function() {
		...
		this.$ok.on("click", function() {
			that.trigger("ok", {
				type: global.DEST_TYPE.ORG_MEMBER,
				data: that.get()
			});
			that.clear();
			that.hide();
		...
		this.listenTo(groupView, "back", function(option) {
			that.show();
		});

		this.listenTo(groupView, "select", function(option) {
			that.trigger("ok", option);
		});
	},
